...
export const fetchCategories = async ({ setGetCategory }: ICatProps) => {
  try {
    const response = await axios.get(
      "http://192.168.29.101:8000/category/getAllCategory"
    );
    ...
    if (Array.isArray(response.data)) {
      const fixedData = response.data.map((item) => ({
        ...
        images: item.images.map((img: string) =>
          img.replace("http://localhost", "http://192.168.29.101")
        ),
      }));
...
export const fetchProductsByCatID = async ({
  ...
  try {
    const response: FetchProductsParam = await axios.get(
      `http://192.168.29.101:8000/product/getProductByCatID/${catID}`
    );
    console.log("API Response:", response.data);
    if (Array.isArray(response.data)) {
      const fixedData = response.data.map((item) => ({
...
export const fetchFeaturedProducts = async ({
  ...
  try {
    const response: FetchProductsParam = await axios.get(
      `http://192.168.29.101:8000/product/getFeaturedProducts/${catID}` // Include catID in the URL
    );
